...
/* xs3: TCC 内置 VFS / UTF-8 文件 IO（移植自 xlang 经验）
 * CONFIG_TCC_BUILTIN_VFS 时 open/fopen 全量重映射到 VFS（内置资源 + 内存挂载
 * + 真实文件回退）；否则 Windows 下重映射为 UTF-8 宽字符 IO，解决中文路径 */
#ifdef CONFIG_TCC_BUILTIN_VFS
# include "tcc_builtin_vfs.h"
# ifndef TCC_BUILTIN_VFS_NO_REMAP
#  define open tcc_vfs_open
#  define close tcc_vfs_close
#  define read tcc_vfs_read
#  define lseek tcc_vfs_lseek
#  define fopen tcc_vfs_fopen
#  define fclose tcc_vfs_fclose
# endif
#elif defined(_WIN32)
# include "tcc_utf8_io.h"
# ifndef TCC_UTF8_IO_NO_REMAP
#  define open tcc_utf8_open
#  define fopen tcc_utf8_fopen
#  define unlink tcc_utf8_unlink
# endif
#endif
