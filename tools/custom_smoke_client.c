...
static int procSendAll(smoke_socket_t hSocket, const char* sText, size_t iLen)
{
	size_t iSent = 0;

	while ( iSent < iLen ) {
		int iRet = send(hSocket, sText + iSent, (int)(iLen - iSent), 0);
		if ( iRet <= 0 ) {
			return -1;
		}
		iSent += (size_t)iRet;
...
int main(int argc, char** argv)
{
	...
	if ( procSendAll(hSocket, sText, strlen(sText)) != 0 ) {
		fprintf(stderr, "send failed\n");
		goto end;
	}

	iRead = recv(hSocket, sBuf, (int)(sizeof(sBuf) - 1u), 0);
	if ( iRead <= 0 ) {
