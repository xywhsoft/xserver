...
{
	/*
	production xs stable surface:
	- keep this list close to the legacy thin-host baseline
	...
	*/
	...
	return
		strcmp(sPath, "/__xs/status") == 0 ||
		strcmp(sPath, "/__xs/status_json") == 0 ||
		strcmp(sPath, "/__xs/health") == 0 ||
		strcmp(sPath, "/__xs/health_json") == 0 ||
		strcmp(sPath, "/__xs/reload") == 0 ||
		strcmp(sPath, "/__xs/reload_json") == 0 ||
		strcmp(sPath, "/__xs/reload_config") == 0 ||
		strcmp(sPath, "/__xs/reload_config_json") == 0 ||
		strcmp(sPath, "/__xs/reload_status") == 0 ||
		strcmp(sPath, "/__xs/reload_status_json") == 0 ||
		strcmp(sPath, "/__xs/check_config") == 0 ||
		strcmp(sPath, "/__xs/check_config_json") == 0;
}

static inline bool XS_DebugManageAPIPath(const char* sPath)
{
	/*
	xsdbg-only surface:
	...
	*/
	...
	return
		...
		strcmp(sPath, "/__xs/ws_metrics") == 0 ||
		strcmp(sPath, "/__xs/ws_metrics_json") == 0 ||
		strcmp(sPath, "/__xs/ws_metrics_clear") == 0 ||
		strcmp(sPath, "/__xs/xtp_metrics") == 0 ||
		strcmp(sPath, "/__xs/xtp_metrics_json") == 0 ||
		strcmp(sPath, "/__xs/xtp_metrics_clear") == 0 ||
		strcmp(sPath, "/__xs/udp_metrics") == 0 ||
		strcmp(sPath, "/__xs/udp_metrics_json") == 0 ||
		strcmp(sPath, "/__xs/udp_metrics_clear") == 0 ||
		strcmp(sPath, "/__xs/custom_metrics") == 0 ||
		strcmp(sPath, "/__xs/custom_metrics_json") == 0 ||
		strcmp(sPath, "/__xs/custom_metrics_clear") == 0;
}
...
static inline bool XS_RuntimeManageAPIEnabled(const XS_ServerConfig* objServer, const XS_HostConfig* objHost, const char* sPath)
{
	...
	return TRUE;
#else
	return XS_CoreManageAPIPath(sPath);
#endif
}

static inline const char* XS_HttpProductionOnlyAPIMessage(const char* sPath)
{
...
static inline bool XS_HttpRespondProductionOnlyDisabled(const char* sPath, xhttpdresponse* pResp)
{
	const char* sMessage;
	...
	sMessage = XS_HttpProductionOnlyAPIMessage(sPath);
	if ( XS_HttpPathExpectsJSONError(sPath) ) {
		return XS_HttpRespondJsonResult(pResp, 403, "Forbidden", FALSE, sMessage);
	}

	return XS_HttpRespondText(pResp, 403, "Forbidden", sMessage);
}
...
static inline const char* XS_HttpDebugOnlyAPIMessage(const char* sPath)
{
	...
	if ( strcmp(sPath, "/__xs/reload_reset") == 0 ) {
		return "config reload reset api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/check_config_clear") == 0 ) {
		return "check config clear api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/http_metrics") == 0 ) {
		return "http metrics api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/http_metrics_json") == 0 ) {
		return "http metrics json api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/http_metrics_clear") == 0 ) {
		return "http metrics clear api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/ws_metrics") == 0 ) {
		return "ws metrics api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/ws_metrics_json") == 0 ) {
		return "ws metrics json api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/ws_metrics_clear") == 0 ) {
		return "ws metrics clear api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/xtp_metrics") == 0 ) {
		return "xtp metrics api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/xtp_metrics_json") == 0 ) {
		return "xtp metrics json api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/xtp_metrics_clear") == 0 ) {
		return "xtp metrics clear api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/udp_metrics") == 0 ) {
		return "udp metrics api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/udp_metrics_json") == 0 ) {
		return "udp metrics json api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/udp_metrics_clear") == 0 ) {
		return "udp metrics clear api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/custom_metrics") == 0 ) {
		return "custom metrics api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/custom_metrics_json") == 0 ) {
		return "custom metrics json api only available in xsdbg";
	}
	if ( strcmp(sPath, "/__xs/custom_metrics_clear") == 0 ) {
		return "custom metrics clear api only available in xsdbg";
	}

	return "debug manage api only available in xsdbg";
}

static inline bool XS_HttpRespondDebugOnlyDisabled(const char* sPath, xhttpdresponse* pResp)
{
	const char* sMessage;
	...
	sMessage = XS_HttpDebugOnlyAPIMessage(sPath);
	if ( XS_HttpPathExpectsJSONError(sPath) ) {
		return XS_HttpRespondJsonResult(pResp, 403, "Forbidden", FALSE, sMessage);
	}
...
static inline bool XS_HttpHostHasResponseHeader(const XS_HostConfig* objHost, const char* sName)
{
	if ( objHost == NULL || sName == NULL || objHost->arrHeaders == NULL || objHost->iHeaderCount == 0u ) {
		return FALSE;
	}
	for ( uint32 i = 0u; i < objHost->iHeaderCount; ++i ) {
		if ( strcasecmp(objHost->arrHeaders[i].sName, sName) == 0 ) {
			return TRUE;
		}
	}
	return FALSE;
...
static inline bool XS_HttpStaticCorsOptionsEnabled(const XS_HostConfig* objHost)
{
	return XS_HttpHostHasResponseHeader(objHost, "Access-Control-Allow-Origin") ||
		XS_HttpHostHasResponseHeader(objHost, "Access-Control-Allow-Methods") ||
		XS_HttpHostHasResponseHeader(objHost, "Access-Control-Allow-Headers");
}

static inline bool XS_HttpStaticMethodAllowed(const XS_HostConfig* objHost, const xhttpdrequest* pReq)
{
	if ( pReq == NULL ) {
		return FALSE;
	}
...
{
	if ( objHost == NULL || pResp == NULL || objHost->arrHeaders == NULL || objHost->iHeaderCount == 0u ) {
		return TRUE;
	}

	for ( uint32 i = 0u; i < objHost->iHeaderCount; ++i ) {
		if ( !xrtHttpdResponseSetHeader(pResp, objHost->arrHeaders[i].sName, objHost->arrHeaders[i].sValue) ) {
			return FALSE;
		}
	}
	return TRUE;
...
static inline XS_HttpStaticResolveResult XS_HttpResolveStaticFilePath(const XS_HostConfig* objHost, const xhttpdrequest* pReq, char** psFilePath)
{
	...
	if ( XS_HttpStaticPathSensitive(sDecodedPath) ) {
		goto denied;
	}
...
static inline bool XS_HttpServeStatic(const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp, xhttpdconn* pConn)
{
	char sLength[32];
	char* sFilePath = NULL;
	size_t iFileSize;
	...
	if ( !XS_HttpStaticMethodAllowed(objHost, pReq) ) {
		xrtHttpdResponseSetHeader(pResp, "Allow", XS_HttpStaticCorsOptionsEnabled(objHost) ? "GET, HEAD, OPTIONS" : "GET, HEAD");
		return XS_HttpRetErrorEx(NULL, objHost, pReq, pResp, NULL, 405, "Method Not Allowed", XS_HttpStaticCorsOptionsEnabled(objHost) ? "static host only supports GET, HEAD or OPTIONS" : "static host only supports GET or HEAD", NULL);
	}
	...
	if ( pConn == NULL || xrtHttpdConnSendFile(pConn, pResp, sFilePath, 0u) != XRT_NET_OK ) {
		xrtFree(sFilePath);
		return XS_HttpRet500Ex(NULL, objHost, pReq, pResp, NULL, "file send failed", NULL);
	}
...
static inline bool XS_HttpHandleScriptHost(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp, xhttpdconn* pConn)
{
	...
	XS_ScriptResponseContext objRespCtx;
	...
	memset(&objRespCtx, 0, sizeof(objRespCtx));
	objRespCtx.iMagic = XS_SCRIPT_RESPONSE_MAGIC;
	objRespCtx.pResp = pResp;
	objRespCtx.pConn = pConn;

	if ( procRequest == NULL ) {
		if ( objHost->Path && objHost->Path[0] ) {
			return XS_HttpServeStatic(objHost, pReq, pResp, pConn);
		}

		return FALSE;
	}

	bHandled = procRequest(objServer, (void*)objHost, &objReqCtx, &objRespCtx);
	if ( bHandled ) {
		return TRUE;
	}
	
	if ( XS_HttpStaticMethodAllowed(objHost, pReq) && objHost->Path && objHost->Path[0] ) {
		XS_LogInfo(
...
static inline bool XS_HttpHandleStatusJson(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp)
{
	...
	xvoTableSetText(objRet, "udp_last_reject_time", sizeof("udp_last_reject_time") - 1, (ptr)(sUdpLastRejectTime ? sUdpLastRejectTime : ""), 0, FALSE);
	xvoTableSetInt(objRet, "udp_last_reject_age_ms", sizeof("udp_last_reject_age_ms") - 1, XS_UdpLastRejectAgeMS());
	xvoTableSetText(objRet, "udp_last_recv_limit_reject_time", sizeof("udp_last_recv_limit_reject_time") - 1, (ptr)(sUdpLastRecvLimitRejectTime ? sUdpLastRecvLimitRejectTime : ""), 0, FALSE);
...
static inline bool XS_HttpHandleCoreManageRequest(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp)
{
	if ( pReq == NULL || pResp == NULL || objServer == NULL || objHost == NULL ) {
		return FALSE;
	}
...
#ifdef XRT_MEM_DEBUG
static inline bool XS_HttpHandleDebugManageRequest(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp)
{
	if ( pReq == NULL || pResp == NULL || objServer == NULL || objHost == NULL ) {
		return FALSE;
	}
...
static inline bool XS_HttpHandleManageRequest(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp)
{
	if ( pReq == NULL || pResp == NULL || objServer == NULL || objHost == NULL ) {
		return FALSE;
	}

	if ( XS_HttpHandleCoreManageRequest(objServer, objHost, pReq, pResp) ) {
		return TRUE;
	}

#ifdef XRT_MEM_DEBUG
	if ( XS_HttpHandleDebugManageRequest(objServer, objHost, pReq, pResp) ) {
		return TRUE;
	}
#endif
	...
	if ( XS_HttpPathReservedManage(pReq->sPath) ) {
		if ( XS_HttpPathExpectsJSONError(pReq->sPath) ) {
			return XS_HttpRespondJsonResult(pResp, 404, "Not Found", FALSE, "manage api not found");
		}
		return XS_HttpRespondText(pResp, 404, "Not Found", "manage api not found");
	}

	return FALSE;
}

static inline bool XS_HttpTryManageRequest(XS_ServerConfig* objServer, const XS_HostConfig* objHost, const xhttpdrequest* pReq, xhttpdresponse* pResp, xhttpdconn* pConn)
{
	if ( pReq == NULL || pResp == NULL || objServer == NULL || objHost == NULL ) {
		return FALSE;
	}
	...
	if (
		XS_HttpPathReservedManage(pReq->sPath) &&
		!XS_RuntimeManageAPIEnabled(objServer, objHost, pReq->sPath) &&
		XS_DebugManageAPIPath(pReq->sPath)
	) {
		return XS_HttpRespondProductionOnlyDisabled(pReq->sPath, pResp);
	}
	...
	return XS_HttpHandleManageRequest(objServer, objHost, pReq, pResp);
}
