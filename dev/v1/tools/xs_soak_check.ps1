...
{
	foreach ( $sPattern in @(
		"ws_smoke_client.soak.*.exe",
		"xtp_pressure_client.soak.*.exe",
		"custom_smoke_client.soak.*.exe"
	) ) {
...
function procBuildCClient([string]$sSourceName, [string]$sOutputBase)
{
	...
	$sOutput = Join-Path $sToolDir ("{0}.soak.{1}.exe" -f $sOutputBase, $sRunTag)
...
function procRunHttpSoak()
{
	$objProc = procStartServer "xs_manage_test.json"

	try {
		$iTotalCount = $iRounds * $iHttpCount
		$iSuccess = 0
		...
		for ( $iRound = 1; $iRound -le $iRounds; $iRound++ ) {
			for ( $i = 1; $i -le $iHttpCount; $i++ ) {
				$objResp = procFetchEx "GET" ("{0}/json?run={1}&round={2}&seq={3}" -f $sManageUrl, $sRunTag, $iRound, $i)
				procRequire ($objResp.status -eq 200) "http request failed at round=$iRound seq=$i status=$($objResp.status)"
				procRequire ($objResp.body -like '*"path":"/json"*') "http body mismatch at round=$iRound seq=$i"
				$iSuccess++
			}

			$tblHealth = procFetchJsonText "$sManageUrl/__xs/health_json"
			procRequire ($tblHealth -like '*"ok":*') "http health_json missing ok at round=$iRound"

			$tblMetric = procFetchJsonText "$sManageUrl/__xs/http_metrics_json"
			$iExpectCount = $iRound * $iHttpCount
			$iAppCount = procGetMetricInt $tblMetric "http_app_req_count"
			procRequire ($iAppCount -eq $iExpectCount) "http_app_req_count expected $iExpectCount, got $iAppCount"
			procRequire ((procGetMetricText $tblMetric "http_last_app_path") -eq "/json") "http_last_app_path mismatch at round=$iRound"

			if ( ($iRound -lt $iRounds) -and ($iPauseMS -gt 0) ) {
				Start-Sleep -Milliseconds $iPauseMS
			}
		}
		...
		return [ordered]@{
			...
			rounds = $iRounds
			requests_per_round = $iHttpCount
			requests = $iTotalCount
			success = $iSuccess
			failures = ($iTotalCount - $iSuccess)
			...
			server_http_app_req_count = (procGetMetricInt $tblMetric "http_app_req_count")
	...
	try {
		...
		for ( $iRound = 1; $iRound -le $iRounds; $iRound++ ) {
			for ( $i = 1; $i -le $iWsCount; $i++ ) {
				$sText = "soak-ws-$iRound-$i"
				$tblRun = procRunClientRetry $script:sWsClientExe @("127.0.0.1", "$iWsPort", $sText)

				procRequire ($tblRun.code -eq 0) "ws client exit=$($tblRun.code) round=$iRound seq=$i text=$($tblRun.text)"
				procRequire ($tblRun.text -like "*ws demo*") "ws body missing demo round=$iRound seq=$i"
				procRequire ($tblRun.text -like "*text=$sText*") "ws body missing echo round=$iRound seq=$i"
				procRequire ($tblRun.text -like "*protocol=xs-demo*") "ws body missing protocol round=$iRound seq=$i"
				$iSuccess++
			}

			$tblMetric = procFetchJsonText "$sManageUrl/__xs/ws_metrics_json"
			$iExpectCount = $iRound * $iWsCount
			procRequire ((procGetMetricInt $tblMetric "ws_open_count") -ge $iExpectCount) "ws_open_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "ws_close_count") -ge $iExpectCount) "ws_close_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "ws_text_count") -ge $iExpectCount) "ws_text_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "ws_conn_current") -eq 0) "ws_conn_current expected 0 after round=$iRound"
		...
		return [ordered]@{
			...
			avg_ms = (procCalcAvgMS $iSuccess $objWatch.ElapsedMilliseconds)
			rps = (procCalcRPS $iSuccess $objWatch.ElapsedMilliseconds)
			server_ws_open_count = (procGetMetricInt $tblMetric "ws_open_count")
			server_ws_close_count = (procGetMetricInt $tblMetric "ws_close_count")
			server_ws_text_count = (procGetMetricInt $tblMetric "ws_text_count")
...
function procRunXtpSoak()
{
	...
	try {
		...
		for ( $iRound = 1; $iRound -le $iRounds; $iRound++ ) {
			$tblRun = procRunClientRetry $script:sXtpClientExe @("127.0.0.1", "$iXtpPort", "$iXtpCount", "demo.ping", "tag=soak-xtp-$sRunTag-$iRound")

			procRequire ($tblRun.code -eq 0) "xtp client exit=$($tblRun.code) round=$iRound text=$($tblRun.text)"
			procRequire ($tblRun.text -like "*ok_count=$iXtpCount*") "xtp ok_count mismatch at round=$iRound"
			procRequire ($tblRun.text -like "*last_status=0*") "xtp last_status mismatch at round=$iRound"
			procRequire ($tblRun.text -like "*last_cmd=xtp.reply*") "xtp last_cmd mismatch at round=$iRound"
			$iSuccess += $iXtpCount

			$tblMetric = procFetchJsonText "$sManageUrl/__xs/xtp_metrics_json"
			$iExpectCount = $iRound * $iXtpCount
			procRequire ((procGetMetricInt $tblMetric "xtp_msg_count") -ge $iExpectCount) "xtp_msg_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "xtp_req_count") -ge $iExpectCount) "xtp_req_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "xtp_send_count") -ge $iExpectCount) "xtp_send_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "xtp_conn_current") -eq 0) "xtp_conn_current expected 0 after round=$iRound"
		...
		return [ordered]@{
			...
			avg_ms = (procCalcAvgMS $iSuccess $objWatch.ElapsedMilliseconds)
			rps = (procCalcRPS $iSuccess $objWatch.ElapsedMilliseconds)
			server_xtp_open_count = (procGetMetricInt $tblMetric "xtp_open_count")
			server_xtp_close_count = (procGetMetricInt $tblMetric "xtp_close_count")
			server_xtp_msg_count = (procGetMetricInt $tblMetric "xtp_msg_count")
			server_xtp_req_count = (procGetMetricInt $tblMetric "xtp_req_count")
			server_xtp_send_count = (procGetMetricInt $tblMetric "xtp_send_count")
...
function procRunCustomSoak()
{
	...
	try {
		$iTotalCount = $iRounds * $iCustomCount
		$iSuccess = 0
		$objWatch = [System.Diagnostics.Stopwatch]::StartNew()
		...
		for ( $iRound = 1; $iRound -le $iRounds; $iRound++ ) {
			for ( $i = 1; $i -le $iCustomCount; $i++ ) {
				$sText = "soak-custom-$iRound-$i"
				$tblRun = procRunClientRetry $script:sCustomClientExe @("127.0.0.1", "$iCustomPort", $sText)

				procRequire ($tblRun.code -eq 0) "custom client exit=$($tblRun.code) round=$iRound seq=$i text=$($tblRun.text)"
				procRequire ($tblRun.text -like "*custom demo*") "custom body missing demo round=$iRound seq=$i"
				procRequire ($tblRun.text -like "*data=$sText*") "custom body missing echo round=$iRound seq=$i"
				$iSuccess++
			}

			$tblMetric = procFetchJsonText "$sManageUrl/__xs/custom_metrics_json"
			$iExpectCount = $iRound * $iCustomCount
			procRequire ((procGetMetricInt $tblMetric "custom_open_count") -ge $iExpectCount) "custom_open_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "custom_close_count") -ge $iExpectCount) "custom_close_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "custom_recv_count") -ge $iExpectCount) "custom_recv_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "custom_send_count") -ge $iExpectCount) "custom_send_count expected >= $iExpectCount"
			procRequire ((procGetMetricInt $tblMetric "custom_conn_current") -eq 0) "custom_conn_current expected 0 after round=$iRound"
		...
		return [ordered]@{
			...
			rounds = $iRounds
			requests_per_round = $iCustomCount
			requests = $iTotalCount
			success = $iSuccess
			failures = ($iTotalCount - $iSuccess)
			elapsed_ms = $objWatch.ElapsedMilliseconds
			avg_ms = (procCalcAvgMS $iSuccess $objWatch.ElapsedMilliseconds)
			rps = (procCalcRPS $iSuccess $objWatch.ElapsedMilliseconds)
			server_custom_open_count = (procGetMetricInt $tblMetric "custom_open_count")
			server_custom_close_count = (procGetMetricInt $tblMetric "custom_close_count")
			server_custom_recv_count = (procGetMetricInt $tblMetric "custom_recv_count")
			server_custom_send_count = (procGetMetricInt $tblMetric "custom_send_count")
...
function procCheckCleanup()
{
	$arrProc = @(Get-Process xs, xsdbg -ErrorAction SilentlyContinue)
	if ( $arrProc.Count -gt 0 ) {
		throw "cleanup_processes remained: " + (($arrProc | ForEach-Object { "$($_.ProcessName):$($_.Id)" }) -join ", ")
	}
	...
	if ( $arrHelper.Count -gt 0 ) {
		throw "generated helper remained: " + (($arrHelper | ForEach-Object { $_.Name }) -join ", ")
	}
}
...
try {
	...
	procWriteSummary "http" (procRunHttpSoak)
	procWriteSummary "ws" (procRunWsSoak)
	procWriteSummary "xtp" (procRunXtpSoak)
	procWriteSummary "custom" (procRunCustomSoak)
