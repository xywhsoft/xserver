...
static unsigned char* procBuildRequestEx(const char* sCmd, XTP_ParamPair* arrPair, int iParamCount, uint64_t iMsgID, size_t* piPackSize)
{
	size_t iCmdSize;
	size_t iPackSize;
	size_t iOffset;
...
static int procReadResponse(xtp_socket_t hSocket, char** psCmd, int32_t* piStatus, char** psBody)
{
	...
	if ( iPackSize < XTP_HEADER_SIZE || iPackSize > (16u * 1024u * 1024u) ) {
		return -1;
	}
	if ( iPackSize > XTP_HEADER_SIZE ) {
		pPack = (unsigned char*)malloc(iPackSize - XTP_HEADER_SIZE);
		if ( pPack == NULL ) {
			return -1;
		}
		if ( procSocketRecvAll(hSocket, pPack, iPackSize - XTP_HEADER_SIZE) != 0 ) {
			free(pPack);
			return -1;
		}
	}
	...
	if ( piStatus ) {
		*piStatus = procLoadI32LE(aHeader + 28u);
	}

	iOffset = (size_t)iParamCount * 4u;
	if ( iOffset + (size_t)iCmdSize + (size_t)iBodySize > (size_t)(iPackSize - XTP_HEADER_SIZE) ) {
		free(pPack);
		return -1;
	}
	...
	for ( i = 0; i < iParamCount; i++ ) {
		uint16_t iKeySize = procLoadU16LE(pPack + ((size_t)i * 4u));
		uint16_t iValueSize = procLoadU16LE(pPack + ((size_t)i * 4u) + 2u);
		iOffset += (size_t)iKeySize + (size_t)iValueSize;
	}
	iOffset -= (size_t)iBodySize;
...
int main(int argc, char** argv)
{
	...
	if ( argc < 5 ) {
		fprintf(stderr, "usage: %s <host> <port> <count> <cmd> [key=value ...]\n", argv[0]);
		return 2;
	}

	sHost = argv[1];
	iPort = (unsigned short)strtoul(argv[2], NULL, 10);
	iCount = (int)strtol(argv[3], NULL, 10);
	sCmd = argv[4];
	if ( iPort == 0u || iCount <= 0 || sCmd == NULL || sCmd[0] == '\0' ) {
		fprintf(stderr, "invalid host/port/count/cmd\n");
		return 2;
	}
	...
	for ( i = 1; i <= iCount; i++ ) {
		...
		pReqBuf = procBuildRequestEx(sCmd, arrPair, iParamCount, (uint64_t)i, &iReqSize);
		if ( pReqBuf == NULL ) {
		...
		if ( procReadResponse(hSocket, &sRespCmd, &iLastStatus, &sRespBody) != 0 ) {
			if ( sRespCmd ) {
				free(sRespCmd);
			}
			if ( sRespBody ) {
		...
		if ( iLastStatus != 0 || sRespCmd == NULL || strcmp(sRespCmd, "xtp.reply") != 0 ) {
			if ( sRespCmd ) {
				free(sRespCmd);
			}
			if ( sRespBody ) {
		...
		snprintf(sLastCmd, sizeof(sLastCmd), "%s", sRespCmd);
		iOKCount++;
